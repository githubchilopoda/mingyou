...
{
    if (!in_array($type, array('best', 'new', 'hot' ,'bk')))
    {
        return array();
    }
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot,g.is_bk, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1 OR g.is_bk = 1)'.
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
					...
					if ($data['is_bk'] == 1)
                    {
                        $goods_data['bk'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'bk'=>'recommend_bk');
        if (empty($type_array))
        {
        ...
        $type_merge = array_merge($type_array['new'], $type_array['best'], $type_array['hot'], $type_array['bk']);
        ...
        foreach ($result AS $idx => $row)
        {
			...
			if (in_array($row['goods_id'], $type_array['bk']))
            {
                $type_goods['bk'][] = $goods[$idx];
            }
        }
    }
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "g.is_best, g.is_new, g.is_hot,g.is_bk, g.is_promote, RAND() AS rnd " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
 ...
 *
 * @access  public
 * @param   string      $type       推荐类型，可以是 best, new, hot,bk, promote
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $num = 0;
    $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'bk'=>'recommend_bk', 'promote'=>'recommend_promotion');
