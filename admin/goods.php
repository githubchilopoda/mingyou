    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            ...
            "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, " .
                    ...
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot,is_bk, " .
                ...
                "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    ...
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_bk', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
        ...
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot,is_bk, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot','$is_bk', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
		...
		/* 设为热销 */
        elseif ($_POST['type'] == 'bk')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'is_bk', '1');
        }

        /* 取消热销 */
        elseif ($_POST['type'] == 'not_bk')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'is_bk', '0');
        }
...
elseif ($_REQUEST['act'] == 'toggle_bk')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_bk         = intval($_POST['val']);

    if ($exc->edit("is_bk = '$is_bk', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_bk);
    }
}
