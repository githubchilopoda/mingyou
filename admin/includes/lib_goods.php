    ...
    if ($result === false)
    {
        ...
        switch ($filter['intro_type'])
        {
            ...
            case 'is_promote':
                ...
                break;
            case 'all_type';
                $where .= " AND (is_best=1 OR is_hot=1 OR is_bk=1 OR is_new=1 OR (is_promote = 1 AND promote_price > 0 AND promote_start_date <= '" . $today . "' AND promote_end_date >= '" . $today . "'))";
        }
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, virtual_sales, shop_price, is_on_sale, is_best, is_new, is_hot,is_bk, sort_order, goods_number, integral, " .
